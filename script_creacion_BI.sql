...
BEGIN
    IF (@tipo NOT IN ('A', 'P'))
        RETURN -1;
    DECLARE @edad INT;
    SET @edad = LOS_DESNORMALIZADOS.BI_obtener_edad(@fecha_nacimiento, @fecha_referencia);
    DECLARE @id_rango INT;
    IF (@tipo = 'A')
        BEGIN
            SELECT TOP 1 @id_rango = id
            FROM LOS_DESNORMALIZADOS.BI_dim_rango_etario_alumnos
            WHERE @edad BETWEEN edad_minima AND edad_maxima;
        END
    ELSE
        BEGIN
            SELECT TOP 1 @id_rango = id
            FROM LOS_DESNORMALIZADOS.BI_dim_rango_etario_profesores
            WHERE @edad BETWEEN edad_minima AND edad_maxima;
        END;
    RETURN @id_rango;
END;
...
BEGIN
    IF EXISTS (
        ...
        WHERE m.curso_id = @codigo_curso
            AND ae.legajo_alumno = @alumno_id
        GROUP BY m.curso_id
        HAVING MIN(ae.nota) >= 4
            AND COUNT(DISTINCT m.id) = (SELECT COUNT(*) FROM LOS_DESNORMALIZADOS.modulo WHERE curso_id = @codigo_curso)
    ) AND EXISTS (
        SELECT 1 FROM LOS_DESNORMALIZADOS.trabajo_practico tp
        WHERE tp.curso_id = @codigo_curso
            AND tp.alumno_id = @alumno_id
            AND tp.nota >= 4
    )
        RETURN 1;
    RETURN 0;
...
BEGIN
    ...
    INSERT INTO LOS_DESNORMALIZADOS.BI_dim_rango_etario_alumnos(descripcion, edad_minima, edad_maxima) VALUES
                                                                                                    ('< 25', 0, 24),
                                                                                                    ('25 - 35', 25, 35),
                                                                                                    ('35 - 50', 36, 50),
                                                                                                    ('> 50', 51, 200);
    INSERT INTO LOS_DESNORMALIZADOS.BI_dim_rango_etario_profesores(descripcion, edad_minima, edad_maxima) VALUES
                                                                                                    ('25 - 35', 25, 35),
                                                                                                    ('35 - 50', 36, 50),
                                                                                                    ('> 50', 51, 200);
    ...
    SELECT s.id, provincia, localidad, s.nombre, direccion, telefono, mail, i.nombre, i.razon_social, i.cuit
    FROM LOS_DESNORMALIZADOS.sede s JOIN institucion i ON s.institucion_id = i.id;
...
BEGIN
    ...
    GROUP BY
        ...
        LOS_DESNORMALIZADOS.BI_obtener_rango_etario(p.fecha_nacimiento, e.fecha_registro, 'P'),
        bs.id;
    PRINT('HECHOS ENCUESTAS MIGRADOS');
END;
GO
...
SELECT
    ...
    t.mes,
    s.nombre AS sede,
    CAST(SUM(CAST(h.es_rechazada AS TINYINT)) AS DECIMAL(10,2)) / COUNT(*) * 100 AS porcentaje_rechazo
FROM LOS_DESNORMALIZADOS.BI_hechos_inscripciones h
         JOIN LOS_DESNORMALIZADOS.BI_dim_tiempo t ON h.tiempo_id = t.id
         JOIN LOS_DESNORMALIZADOS.BI_dim_sede s ON h.sede_id = s.id
...
SELECT
    t.anio,
    s.nombre AS sede,
    CAST(SUM(CAST(h.cursada_aprobada AS TINYINT)) AS DECIMAL(10,2)) / COUNT(*) * 100 AS porcentaje_aprobacion_cursada
FROM LOS_DESNORMALIZADOS.BI_hechos_inscripciones h
         JOIN LOS_DESNORMALIZADOS.BI_dim_tiempo t ON h.tiempo_id = t.id
         JOIN LOS_DESNORMALIZADOS.BI_dim_sede s ON h.sede_id = s.id
...
SELECT
    ...
    t.anio,
    CASE WHEN t.mes <= 6 THEN 1 ELSE 2 END AS semestre,
    CAST(SUM(CAST(h.es_ausente AS TINYINT)) AS DECIMAL(10,2)) / COUNT(*) * 100 AS porcentaje_ausentismo
FROM LOS_DESNORMALIZADOS.BI_hechos_finales h
         JOIN LOS_DESNORMALIZADOS.BI_dim_tiempo t ON h.tiempo_examen_id = t.id
         JOIN LOS_DESNORMALIZADOS.BI_dim_sede s ON h.sede_id = s.id
...
SELECT
    t.anio,
    CASE WHEN t.mes <= 6 THEN 1 ELSE 2 END AS semestre,
    CAST(SUM(CAST(h.es_pago_fuera_termino AS TINYINT)) AS DECIMAL(10,2)) / COUNT(*) * 100 AS porcentaje_pagos_tardios
